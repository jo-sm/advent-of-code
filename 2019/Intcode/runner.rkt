...
(define (get-opcode program pos)
  ...
  (define num
    (string->number (substring stringified-opcode (max 0 (- (string-length stringified-opcode) 2)))))
...
(define (translate-arg-pos-to-args program pos args-pos)
  ...
  (define (iter result remaining-params args-pos)
    (cond
      ...
      [(= (car remaining-params) 0)
       (iter (cons (cons 0 (car args-pos)) result) (cdr remaining-params) (cdr args-pos))]
      [(= (car remaining-params) 1)
       (iter (cons (cons 1 (car args-pos)) result) (cdr remaining-params) (cdr args-pos))]
      [(= (car remaining-params) 2)
       (iter (cons (cons 2 (car args-pos)) result) (cdr remaining-params) (cdr args-pos))]))
    ...
    (generator ()
               ...
               (let loop ([result (list CONTINUE program (void) null 0 0)])
                 (cond
                   ...
                   [else (let ([input (yield (list (fourth result)))])
                           (loop (run-instruction (second result)
                                                  input
                                                  (fourth result)
                                                  (fifth result)
                                                  (sixth result))))]))))
