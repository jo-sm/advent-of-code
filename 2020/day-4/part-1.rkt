...
(define raw-passports (map string-split (read-input-file #:file-parser (cut string-split <> "\n\n"))))

; List of processed passports, e.g. (("iyr" "2010" ("ecl" "gry")))
(define passports
  ; Map over each "raw" passport, and split each of its keys by ":"
  (map (lambda (raw-passport)
         (map (lambda (passport-item) (string-split passport-item ":")) raw-passport))
       raw-passports))
