  ...
  (define (and-inner str)
    ...
    (for/fold ([passed #t] [remaining str] #:result (and passed remaining)) ([fn fns])
      ...
      (if (not passed)
          ...
          (let ([next (fn remaining)]) (if next (values #t next) (values #f ""))))))
...
(define (_or fns)
  (define (or-inner str)
    ...
    (for/fold ([result str] [passed #f] #:result (and passed result)) ([fn fns])
      ; (displayln (list "-> (or) fn" result passed fn))
      (if passed
          (values result passed)
          (let ([next (fn result)]) (if next (values next #t) (values result passed))))))
...
(define/memo (startswith piece)
             (define (startswith-inner s)
               ...
               (and (string-prefix? s piece) (substring s (string-length piece))))
             startswith-inner)

(define/memo
 (parse-rule rules rule)
 (_or (for/list ([subrule rule])
        (if (string? subrule)
            (lambda (s) (and (string-prefix? s subrule) (substring s (string-length subrule))))
