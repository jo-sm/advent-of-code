...
(define hands
  (let* ([raw (string-split (file->string "input") "\n\n")]
    ...
    (map (λ (hand-str) (map string->number hand-str)) hands-strs)))

(define (play-game hand1 hand2)
  (let loop ([p1-hand hand1] [p2-hand hand2] [p1-prev-hands (set)] [p2-prev-hands (set)])
    (cond
      [(or (empty? p1-hand) (empty? p2-hand)) (list p1-hand p2-hand)]

      ; If we would repeat this configuration, player1 wins immediately
      [(and (set-member? p1-prev-hands p1-hand) (set-member? p2-prev-hands p2-hand))
       (list p1-hand '())]

      [(and (> (length p1-hand) (car p1-hand)) (> (length p2-hand) (car p2-hand)))
